...
<?php
	...
	if($_GET['func'] == 1)
	{
		...
		if(strlen($descri) > 50)
		{
			die("Descrição do Avatar com mais de 50 Dígitos! <br /><br /><input type=\"button\" value=\"Tentar novamente\" onClick=\"location.href='avatar.php?personagem=$personagem&login=$login&senha=$senha_md5'\" />");
		}
		...
		{
			die("Arquivo em formato inválido! A imagem deve ser jpg, jpeg, bmp, gif ou png. Envie outro arquivo! <br /><br /><input type=\"button\" value=\"Tentar novamente\" onClick=\"location.href='avatar.php?personagem=$personagem&login=$login&senha=$senha_md5'\" />");
		}

		if($arquivo["size"] > $config["tamanho"])
		{
			die("Muito Grande, deve ser menor que " . $config["tamanho"] . " bytes <br /><br /><input type=\"button\" value=\"Tentar novamente\" onClick=\"location.href='avatar.php?personagem=$personagem&login=$login&senha=$senha_md5'\" />");
		}
		
		if($tamanhos[0] > $config["largura"])
        {
            die("Largura da imagem não deve ultrapassar " . $config["largura"] . " pixels <br /><br /><input type=\"button\" value=\"Tentar novamente\" onClick=\"location.href='avatar.php?personagem=$personagem&login=$login&senha=$senha_md5'\" />");
        }
		
		if($tamanhos[1] > $config["altura"])
        {
            die("Altura da imagem não deve ultrapassar " . $config["largura"] . " pixels <br /><br /><input type=\"button\" value=\"Tentar novamente\" onClick=\"location.href='avatar.php?personagem=$personagem&login=$login&senha=$senha_md5'\" />");
        }
