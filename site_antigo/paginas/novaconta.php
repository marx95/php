...
{	
	...
	if($captchaSv != $captcha)	die("<span id='Falha'>Código de verificação invalído!</span>");
	if(empty($login))			die("<span id='Falha'>Login em branco!</span>");
	if(empty($senha))			die("<span id='Falha'>Senha em branco!</span>");
	if(empty($nome))			die("<span id='Falha'>Nome em branco!</span>");
	if(empty($resenha))			die("<span id='Falha'>Resenha em branco!</span>");
	if(empty($email))			die("<span id='Falha'>Email em branco!</span>");
	if(empty($Set_Premio))		die("<span id='Falha'>Selecione o Prêmio do cadastro!</span>");
	if(strlen($login) < 4) 		die("<span id='Falha'>Login mínimo 4 dígitos!</span>");
	if(strlen($senha) < 4)		die("<span id='Falha'>Senha mínimo 4 dígitos!</span>");
	if(strlen($resenha) < 4)	die("<span id='Falha'>Resenha mínimo 4 dígitos!</span>");
	if(strlen($email) < 10)		die("<span id='Falha'>Email mínimo 10 dígitos!</span>");
	if($Set_Premio == 0)		die("<span id='Falha'>Selecione o Prêmio do cadastro!</span>");
	...
	<span id='Falha'>Email inválido, disponivel apenas nos sequintes servidores:</span><br />
	...
	<span id='Falha'>@munovus.net</span>");
	...
	if($ativar_via_email == 0)
	{
		...
		if($vi_curr_info == 1) mssql_query("INSERT INTO [$db_mssql].[dbo].[VI_CURR_INFO](ends_days, chek_code, used_time, memb___id, memb_name, memb_guid, sno__numb, Bill_Section, Bill_value, Bill_Hour, Surplus_Point, Surplus_Minute, Increase_Days )  VALUES ('2013', '1', '1234', '$login', '$nome', 1, '7', '6', '3', '6', '6', '2012-00-00 00:00:00', '0' )");
...
<table>
	...
	<td colspan="2">Cadastro</td>
	  ...
	  <tr>
        ...
        <td>
          <input name="nome" type="text" id="nome" maxlength="20" style="text-transform: lowercase;" tabindex="4" />
        </td>
    </tr>
	  <tr>
  ...
  <tr>
    ...
    <tr>
    <td onmousemove="ToolMsg('Bronze Set +9 Semi-Full & Blade +13 Full')"><img src="img/dk.jpg" onclick="document.getElementById('set_premio').value = 1;" style="cursor: pointer;" /></td>
    <td onmousemove="ToolMsg('Sphinx Set +9 Semi-Full & Serpent Staff +13 Full')"><img src="img/dw.jpg" onclick="document.getElementById('set_premio').value = 2;" style="cursor: pointer;" /></td>
    <td onmousemove="ToolMsg('Silk Set +9 Semi-Full & Battle Bow +13 Full')"><img src="img/elf.jpg" onclick="document.getElementById('set_premio').value = 3;" style="cursor: pointer;" /></td>
    </tr>
    <tr>
    <td colspan="3">
    <p>
  ...
  ?>
    <tr>
	    <td width="50%">
			...
			<span style="font-weight:bold; letter-spacing: 1px; cursor:default;"><?php echo $captcha; ?></span></td></td>
	    <td>
  ...
  <table>
  <tr>
	<td colspan="2"><?php echo $vantagens; ?></td>
	</tr>
  <tr><td><input type="submit" name="Submit" value="Cadastrar" tabindex="7"/></td></tr></table>
</form>
